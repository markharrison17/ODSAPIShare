...
class ODSFileManager(object):
    ...
    def __init__(self, xml_file_path, schema_file_path, xml_url=None, schema_url=None):
        ...
        # If the xml_url has been passed in to the constructor,
        # we will retrieve the zip file from the remote url
        if xml_url:
        ...
        # Otherwise we will set local_mode which will skip the download and just look for
        # the zip file locally
        else:
    ...
    def __retrieve_latest_schema_file(self):
        ...
        # If running in local mode, we simply check that the zip file is already present locally
    ...
    def __retrieve_latest_schema(self):
        ...
        try:
            ...
            print(schema_filename, "SCHEMA FILENAME")
    ...
    def get_latest_xml(self):
        """Check if we have ODS xml data. If not retrieve the latest version available and
