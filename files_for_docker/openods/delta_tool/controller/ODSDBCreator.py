...
from import_tool.models.Contact import Contact
...
class ODSDBCreator(object):
    ...
    def __create_codesystems(self):
        ...
        for code_system_type in code_system_types:
            ...
            for idx, relationship in enumerate(relationships.findall('concept')):
                ...
                # pop these in a global dictionary, we will use these later in create_organisations
    ...
    def __create_organisations(self):
        ...
        for idx, organisation in tqdm(enumerate(self.__ods_xml_data.findall(
            ...
            organisations[idx].record_class = organisation.attrib.get('orgRecordClass')
            ...
            self.__create_contacts(organisations[idx], organisation)
    ...
    def __create_contacts(self, organisation, organisation_xml):
        """Creates the contacts, this should only be called from
        ...
        None
        """
        contacts_xml = organisation_xml.find('Contacts')
        contacts = {}
        ...
        if contacts_xml is not None:
            ...
            for idx, contact in enumerate(contacts_xml):
                ...
                contacts[idx].organisation_ref = organisation.ref
                contacts[idx].org_odscode = organisation.odscode
                contacts[idx].type = contact.attrib.get('type')
                contacts[idx].value = contact.attrib.get('value')
                self.session.add(contacts[idx])
    ...
    def __create_relationships(self, organisation, organisation_xml):
        ...
        if relationships_xml is not None:
            ...
            for idx, relationship in enumerate(relationships_xml):
                ...
                try:
                    relationships[idx].target_primary_role_code = \
                        relationship.find('Target/PrimaryRoleId').attrib.get('id')
                except AttributeError:
                    pass

                try:
                    relationships[idx].target_unique_role_id = \
    ...
    def __create_addresses(self, organisation, organisation_xml):
        ...
        location = organisation_xml.find('GeoLoc/Location')
    ...
    def __create_successors(self, organisation, organisation_xml):
        ...
        for idx, succ in enumerate(organisation_xml.findall(
            ...
            try:
                successors[idx].unique_id = succ.attrib.get('uniqueSuccId')
            except AttributeError:
                pass

            try:
                successors[idx].org_odscode = organisation.odscode
            except AttributeError:
                pass

            try:
                successors[idx].legal_start_date = \
